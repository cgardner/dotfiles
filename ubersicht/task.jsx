...
const getOffsetClass = (due) => {
  ...
  if (offset === 0) {
    return css`color: rgb(255,200,0)`;
  }
}

const Task = ({task, index}) => {
  const {tags = [], id, description, project="", urgency, priority, due} = task;
...
export const render = ({output}) => {
  ...
  return (<table className={tableClass}>
    ...
    <tbody>
      {jsonOutput.map((task, i) => (<Task key={i} task={task} index={i} />))}
    </tbody>
  </table>);
}
...
`
const priorityClass = {
  H: css`color: rgb(150,200,255)`,
  M: css`color: rgb(0,150,255)`,
  L: css`color: rgb(0,0,255)`
};
